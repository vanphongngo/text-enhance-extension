...
(async function () {
  ...
  const initTextEnhancer = () => {
    ...
    const originalTextElem =
      originalTextContainer.querySelector(".original-text");

    // Initialize refinement button with loading state
    const setRefineButtonLoading = (isLoading) => {
      if (isLoading) {
        refineButton.innerHTML =
          '<span class="loading-spinner"></span> Refining...';
    ...
    popupIcon.addEventListener("click", async function (e) {
      ...
      if (!cachedSelectionState) {
        ...
        originalTextElem.textContent =
          cachedSelectionState.text || currentSelection;
      ...
      try {
        ...
        const textToEnhance = cachedSelectionState
          ? cachedSelectionState.text
          : currentSelection;
