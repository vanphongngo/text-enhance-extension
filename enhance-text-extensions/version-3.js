  ...
  const initTextEnhancer = () => {
    ...
    document.addEventListener('mouseup', function(e) {
      setTimeout(() => {
        ...
        // Store the selection info and element
        currentInputElement = activeElement;
        currentSelection = selectedText;
        
        // Store selection positions for later use
        if (activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA') {
          selectionStart = activeElement.selectionStart;
          selectionEnd = activeElement.selectionEnd;
        } else {
          // For contenteditable, we'll handle this differently later
    ...
    // Function to safely replace text in the current input element
    const replaceSelectedText = (newText) => {
      if (!currentInputElement) return;
      
      try {
        // Handle standard input elements (input, textarea)
        if (currentInputElement.tagName === 'TEXTAREA' || currentInputElement.tagName === 'INPUT') {
          const currentValue = currentInputElement.value;
          
          // Ensure we have valid selection points
          if (typeof selectionStart !== 'number' || typeof selectionEnd !== 'number') {
            console.error('Invalid selection positions for input element');
            return;
          }
          ...
          // Set focus back to the element and select the new text
          currentInputElement.focus();
          currentInputElement.selectionStart = selectionStart;
          currentInputElement.selectionEnd = selectionStart + newText.length;
        } 
        // Handle contenteditable elements
        else if (currentInputElement.getAttribute('contenteditable') === 'true') {
          // For contenteditable we need to:
          // 1. Get the current selection
          const selection = window.getSelection();
          
          // 2. Check if there's still a selection
          if (selection.rangeCount === 0) {
            // Create a new range at the last known position
            ...
            // Collect all text nodes in the contenteditable
            collectTextNodes(currentInputElement);
            ...
            // Look through text nodes to find our position
            ...
            // If we found the positions, set the range
            if (startNode && endNode) {
              ...
              // Fallback: just use execCommand
              document.execCommand('insertText', false, newText);
              return;
            }
          }
          
          // 3. Now replace the selected text
          document.execCommand('insertText', false, newText);
        }
        
        // Hide the popup after successful replacement
        popupContent.style.display = 'none';
        popupIcon.style.opacity = '0';
        popupIcon.style.pointerEvents = 'none';
